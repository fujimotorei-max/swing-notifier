...
def run(mode="intraday"):
    ...
    for code, name in watchlist.items():
        ...
        df = yf.download(code, period="200d", interval=interval, progress=False)
        if df.empty:
            print("データなし")
            continue
        ...
        # =====================
            ...
            if len(df) >= 2:
                ...
                curr = df.iloc[-1]
                if (
                    prev["SMA5"] <= prev["SMA25"] or prev["SMA25"] <= prev["SMA75"]
                ) and (curr["SMA5"] > curr["SMA25"] > curr["SMA75"]):
